...
@Component({
  selector: 'app-how-many-vaccines-expired',
  templateUrl: './how-many-vaccines-expired.component.html',
  styleUrls: ['./how-many-vaccines-expired.component.css']
})
export class HowManyVaccinesExpiredComponent implements OnInit {
  manyProducer: any;
  pullojaYhteensaKaikki: number;
  rokotteitaYhteensaKaikki: number;
  rokotteetKaytetytKaikki: number;
  rokotteetVanhentuneetKaikki: number;
  paiva: string;
  ...
  constructor(
    ...
    this.rokotteetVanhentuneetKaikki = 0;
  ...
  howManyVaccinesExpired(f: NgForm) {
    ...
    if (this.pullojaYhteensaKaikki === 0 && this.rokotteitaYhteensaKaikki === 0 && this.rokotteetKaytetytKaikki === 0 && this.rokotteetVanhentuneetKaikki === 0) {
      console.log('alkoi')
    ...
    this.vaccineService.howManyVaccinesExpired(f.value.date).subscribe((data) => {
      ...
      for (let x = 0; x < data.length; x++) {
        ...
        this.rokotteetVanhentuneetKaikki = this.rokotteetVanhentuneetKaikki + data[x].rokotteetVanhentuneet;
      }
